...
weight_drm = drm(data=data, weight_g~GDD, crop, fct =W1.3(fixed = c(NA,350,NA), names = c("slope","upper", "ec50")))
summary(weight_drm)

plot(weight_drm, type="b")
```
...
```{r message=FALSE, warning=FALSE, include=FALSE}
newdata <- expand.grid(GDD=exp(seq(log(168), log(748), length=748)))
...
ggplot(data, aes(x=GDD, y=weight_g, fill = crop))  +
  ...
  geom_ribbon(data=nd, aes(x=GDD, y=p, ymin=pmin, ymax=pmax), alpha=0.8) +
